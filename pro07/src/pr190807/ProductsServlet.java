...
public class ProductsServlet extends HttpServlet {
	...
	protected void doHandler(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command!=null && command.equals("showProd")){
			if(input_prod==null) {
				input_prod="";
			}
			if(input_vend==null) {
		...
		}else if(command!=null && command.equals("delProd")) {
		...
		if(list.isEmpty()) {
			...
			for(int i=0; i<list.size();i++) {//조회한 회원 정보를 for문과 <tr>태그를 이용해 리스트로 출력합니다.
				...
				out.print("<tr><td>"+prod_id+"</td><td>"+prod_name+"</td><td>"+prod_price
						...
						+"<td><a href='/pro07/products?command=delProd&del_id="+prod_id+"&prod_id="+input_prod+"&vend_id="+input_vend+"'>삭제</a></td></tr>");
			}
			out.print("</table>");
		}
		out.print("</body></html>");
		out.print("<a href='/pro07/products.html'>다시 조회하기</a>");
	}
